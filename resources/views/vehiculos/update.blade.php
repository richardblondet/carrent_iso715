			...
			<form action="{{ route('vehiculos.update', $vehiculo->id ) }}" method="post">
				...
				<div class="form-group">
					...
					<select name="marca_id" id="marca_id" class="form-control" required>
						...
						<option value="{{ $marca->id }}" {{ $vehiculo->marcar_id == $marca->id ? 'selected="selected"' : '' }}>{{ $marca->nombre }}</option>
						@empty
				...
				<div class="form-group">
					...
					<select name="modelo_id" id="modelo_id" class="form-control" required >
						...
						<option value="{{ $modelo->id }}" {{ $vehiculo->modelo_id == $modelo->id ? 'selected="selected"' : '' }} data-marca="{{ $modelo->marca->id }}" data-marca-name="{{ $modelo->marca->nombre }}">{{ $modelo->modelo }}</option>
						@empty
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" id="year" aria-describedby="rolHelp" name="ano" value="{{ $vehiculo->ano }}">
				</div>
				<div class="form-group">
					...
					<select name="tipo_vehiculo_id" id="tipo_vehiculo_id" class="form-control" required>
						...
						<option value="{{ $tipo->id }}" {{ $vehiculo->tipo_vehiculo_id == $tipo->id ? 'selected="selected"' : '' }}>{{ $tipo->nombre }}</option>
						@empty
				...
				<div class="form-group">
					...
					<select name="tipo_combustible_id" id="tipo_combustible_id" class="form-control" required>
						...
						<option value="{{ $combustible->id }}" {{ $vehiculo->tipo_combustible_id == $combustible->id ? 'selected="selected"' : '' }}>{{ $combustible->nombre }}</option>
						@empty
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" id="chassis" aria-describedby="rolHelp" name="chassis" value="{{ $vehiculo->chassis }}">
				</div>
				<div class="form-group">
					<label for="motor">No Motor</label>
					<input type="text" class="form-control" id="motor" aria-describedby="rolHelp" name="numero_motor" value="{{ $vehiculo->numero_motor }}">
				</div>
				<div class="form-group">
					<label for="nombreRol">Nombre</label>
					<input type="text" class="form-control" id="nombreRol" aria-describedby="rolHelp" name="nombre" value="{{ $vehiculo->nombre }}">
				</div>
				<div class="form-group">
					<label for="placa">Placa</label>
					<input type="text" class="form-control" id="placa" aria-describedby="rolHelp" name="placa"  value="{{ $vehiculo->placa }}">
				</div>
				<div class="checkbox checkbox-circle checkbox-info peers ai-c mB-15">
					<input type="checkbox" id="estadoRol" name="estado" class="peer"  {{ $vehiculo->estado ? 'checked="checked"' : '' }} >
